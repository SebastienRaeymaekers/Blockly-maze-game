...
class KeyEventItem extends RecordingItem {
    ...
    @Override
    void replay(int itemIndex, CanvasWindow window) {
        window.handleKeyEvent(id, keyCode, keyChar, e);
    }
}
...
public class CanvasWindow {
    ...
    private void handleMouseEvent_(MouseEvent e) {
        //System.out.println(e); --> ik heb dit zelf uitgecomment, anders spamt het alles
        if (recording != null)
    ...
    private void handleKeyEvent_(KeyEvent e) {
        ...
        handleKeyEvent(e.getID(), e.getKeyCode(), e.getKeyChar(), e); // we pass the keyEvent as well to detect .isControlDown() and such
    }
    ...
    class Panel extends JPanel {

        {
            ...
            // custom resize code from here on
            addComponentListener(new ComponentAdapter(){

                @Override
                public void componentResized(ComponentEvent e) {
                    widthScale = e.getComponent().getWidth() * 1.0 / width;
                    heightScale = e.getComponent().getHeight() * 1.0 / height;
                    Panel.super.repaint();
                }
            });
        }
